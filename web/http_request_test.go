...
func TestHTTPRequest_GET(t *testing.T) {
	...
	is.Equal(resp.Headers().Values(), []string{"content-length: 703", "content-type: application/json"})
}
...
func TestHTTPRequest_POST(t *testing.T) {
	...
	is.Equal(resp.Status(), "")
	is.Equal(resp.Headers().Get("Content-Type"), "application/json")
	is.Equal(resp.Headers().Values(), []string{"content-length: 807", "content-type: application/json"})
